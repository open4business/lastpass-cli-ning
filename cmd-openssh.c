...
static void start_putty(char *user, char *url, char *password)
{ 	 
	...
	struct buffer buf;
	buffer_init(&buf); 
	buffer_append_str(&buf,"putty.exe ");	
	buffer_append_str(&buf,user);	
	buffer_append_str(&buf,"@");	
	buffer_append_str(&buf,url);

	//Sonderfall gitlab01 und gitlab02: geht über Port 2222	
	if (strcmp(url, "gitlab01") == 0 || strcmp(url, "gitlab02")==0) 
		buffer_append_str(&buf," 2222 -pw ");
	 else
		buffer_append_str(&buf," 22 -pw ");
	
	buffer_append_str(&buf,password);  
	HANDLE  hThread;
	hThread = CreateThread( 
            NULL,                   // default security attributes
            0,                      // use default stack size  
            MyThreadFunction,       // thread function name
            buf.bytes,          // argument to thread function 
...
//	struct attach *attach;
	...
	if (!field_format) {
		field_format = xstrdup(
			TERMINAL_FG_YELLOW "f%n" TERMINAL_RESET ": %fv");
	}
	...
	list_for_each_entry(account, &matches, match_list) {
		...
		if (choice == ALL) {
		//	print_header(title_format, found);
			start_putty(found->username, found->url, found->password);
		/*
